...
def SGD_Model(tup,sc):
    ...
    try: 
        if model_flag == 0:
            ...
            model_flag = 1
            print("1st iteration of SGDlog Model has began")
            model = SGDClassifier(loss='log')
            model.partial_fit(Xaxis_train,Yaxis_train.ravel(),classes=np.unique(Yaxis_train))
            pred_batch = model.predict(Xaxis_test)
            joblib.dump(model, 'weights/SGDlog.pkl')
            
        else:
        
            max_f1score_flag=1
            print("Increamental learning of SGDlog Model has began")
            model_load = joblib.load('weights/SGDlog.pkl')
            model_load.partial_fit(Xaxis_train,Yaxis_train.ravel())
            pred_batch = model_load.predict(Xaxis_test)
            joblib.dump(model_load, 'weights/SGDlog.pkl')
        ...
        if max_f1score < fscore:
            ...
            if max_f1score_flag == 1:
                joblib.dump(model_load,'max_SGD.pkl')
            else :
                joblib.dump(model,'max_SGD.pkl')
        csv_writer(accuracy , fscore , precision, recall , score , max_f1score, 'SGD_Log')
        print("\n iteration ended \n")
