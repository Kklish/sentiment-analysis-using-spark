...
def multinomial_model(tup,sc):
    ...
    try: 
        if model_flag == 0:
            ...
            model_flag = 1
            print("1st iteration of MNB Model has began")
            model = MultinomialNB()
            model.partial_fit(Xaxis_train,Yaxis_train.ravel(),classes=np.unique(Yaxis_train))
            pred_batch = model.predict(Xaxis_test)
            joblib.dump(model, 'weights/MNB.pkl')
            
        else:
        
            max_f1score_flag=1
            print("Increamental learning of MNB Model has began")
            ...
            joblib.dump(model_load, 'weights/MNB.pkl')
        ...
        if max_f1score < fscore:
            ...
            if max_f1score_flag == 1:
                joblib.dump(model_load,'max_MNB.pkl')
            else :
                joblib.dump(model,'max_MNB.pkl')
        csv_writer(accuracy , fscore , precision, recall , score , max_f1score, 'MNB')
        print("\n iteration ended \n")
